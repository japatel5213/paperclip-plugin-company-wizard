...
| `aiProvider` | No | Which AI backend powers the wizard interview: `anthropic` (default), `gemini`, `openai`, or `opencode`. |
| `anthropicApiKey` | If `anthropic` | Anthropic API key. Stored as a secret ref. |
| `geminiApiKey` | If `gemini` | Google Gemini API key (aistudio.google.com). Stored as a secret ref. |
| `geminiModel` | No | Gemini model. Default `gemini-2.0-flash`. |
| `openaiApiKey` | If `openai` | OpenAI or OpenRouter API key. Stored as a secret ref. |
| `openaiBaseUrl` | No | OpenAI-compatible base URL. Default `https://api.openai.com/v1`. OpenRouter: `https://openrouter.ai/api/v1`. |
| `openaiModel` | No | OpenAI/OpenRouter model. Default `gpt-4o-mini`. |
| `opencodeBaseUrl` | No | URL of your `opencode serve` instance. Default `http://localhost:4096`. |
| `opencodeModel` | If `opencode` | OpenCode model id in `provider/model` form, e.g. `anthropic/claude-sonnet-4`. |
| `opencodeUsername` | No | Basic-auth user for OpenCode server. Default `opencode`. |
| `opencodePassword` | No | Basic-auth password (matches `OPENCODE_SERVER_PASSWORD`). Stored as a secret ref. |
...
This fork adds **multi-provider support** for the wizard interview step. Set `aiProvider` in plugin settings and fill in the matching fields.

### `anthropic` (default)
Claude via the Anthropic API. Set `anthropicApiKey`. Requires a positive credit balance at console.anthropic.com.

### `gemini`
Google Gemini via the Generative Language API. Set `geminiApiKey` (free tier at aistudio.google.com) and optionally `geminiModel` (default `gemini-2.0-flash`). Messages are converted to Gemini's `contents`/`systemInstruction` format automatically.

### `openai`
Any OpenAI-compatible chat-completions API. Set `openaiApiKey`, optionally `openaiBaseUrl` and `openaiModel`.
- **OpenAI:** leave `openaiBaseUrl` default, use a model like `gpt-4o-mini`.
- **OpenRouter:** set `openaiBaseUrl` to `https://openrouter.ai/api/v1` and a model like `google/gemini-2.0-flash-exp:free`.
- **Local (LiteLLM / vLLM / LM Studio / Ollama):** point `openaiBaseUrl` at your local `/v1` endpoint.

### `opencode`
A self-hosted [OpenCode](https://opencode.ai) server started with `opencode serve`. It exposes a chat-completions surface and can route to cloud or local models.

```bash
# On the server, in tmux:
OPENCODE_SERVER_PASSWORD=your-secret opencode serve --port 4096 --hostname 0.0.0.0
```

Then set: `aiProvider=opencode`, `opencodeBaseUrl=http://localhost:4096`,
`opencodeModel=anthropic/claude-sonnet-4` (or any model configured in OpenCode),
and `opencodePassword=your-secret` (omit if the server is unauthenticated).

> The wizard sends requests to `${opencodeBaseUrl}/chat/completions`. Confirm your OpenCode version exposes that surface via `curl http://localhost:4096/doc`; adjust `opencodeBaseUrl` if your build mounts it under a different path.
...
```bash
git clone https://github.com/<your-username>/paperclip-plugin-company-wizard.git
cd paperclip-plugin-company-wizard
bash install.sh
```

`install.sh` builds the plugin, backs up and replaces the copy under
`~/.paperclip/plugins/node_modules/@yesterday-ai/paperclip-plugin-company-wizard`,
and restarts the `paperclip` tmux session. Override defaults with env vars:

```bash
PAPERCLIP_TMUX=paperclip PAPERCLIP_RUN_CMD="npx paperclipai run" bash install.sh
```

> The original plugin must have been installed once from the Paperclip UI so the
> target directory exists. The script keeps a timestamped `.bak-*` backup and
