...
- **Multi-provider AI wizard** — the interview/quick-generate step can now run on four backends, selected via the new `aiProvider` setting (`anthropic` | `gemini` | `openai` | `opencode`):
  - **Gemini** — Google Generative Language API (`callGemini`), with automatic conversion of messages to Gemini's `contents`/`systemInstruction` format. Configured via `geminiApiKey` and `geminiModel` (default `gemini-2.0-flash`).
  - **OpenAI / OpenRouter / local** — any OpenAI-compatible `/chat/completions` endpoint (`callOpenAI`), via `openaiApiKey`, `openaiBaseUrl` (default `https://api.openai.com/v1`), and `openaiModel` (default `gpt-4o-mini`).
  - **OpenCode** — a self-hosted `opencode serve` instance (`callOpenCode`) with optional HTTP basic auth (`opencodeUsername`/`opencodePassword`), via `opencodeBaseUrl` (default `http://localhost:4096`) and `opencodeModel`.
- **Provider-aware config validation** — `check-ai-config` validates the key/field matching the selected provider and returns the active `provider`.
- **Provider badge** in the wizard's Describe step showing which backend is active.
- **`install.sh`** — one-command build + install into Paperclip's plugin dir with timestamped backup and tmux restart.

### Changed

- `ai-chat` worker action now routes through `callAiProvider()` instead of calling Anthropic directly.
- Package renamed to `@tec-industries/paperclip-plugin-company-wizard`; removed npm-publish and husky `prepare` hooks for fork/self-host installs.
