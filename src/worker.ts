  ...
  async setup(ctx) {
    ...
    async function callOpenAI(
      ...
      const response = await fetch(`${baseUrl}/chat/completions`, {
        ...
        body: JSON.stringify({ model, max_tokens: 8192, messages: allMessages }),
      });
      if (!response.ok) {
        const body = await response.text();
    ...
    /**
     * Call an OpenCode server (https://opencode.ai).
     * OpenCode runs a headless server via `opencode serve` (default port 4096).
     * It exposes an OpenAI-compatible /chat/completions style surface and can be
     * protected with HTTP basic auth via OPENCODE_SERVER_PASSWORD
     * (username defaults to "opencode" or OPENCODE_SERVER_USERNAME).
     *
     * We treat it as an OpenAI-compatible endpoint but inject a Basic auth header
     * when a password is configured, instead of a Bearer token.
     */
    ...
    async function callAiProvider(
      ...
      if (provider === 'gemini') {
        ...
        if (!apiKey) throw new Error('Gemini API key not configured. Add geminiApiKey in plugin settings.');
      ...
      if (provider === 'openai') {
        ...
        if (!apiKey) throw new Error('OpenAI API key not configured. Add openaiApiKey in plugin settings.');
        const baseUrl = (cfg.openaiBaseUrl || 'https://api.openai.com/v1').replace(/\/$/, '');
        const model = cfg.openaiModel || 'gpt-4o-mini';
        return callOpenAI(apiKey, baseUrl, model, messages, system);
      }

      if (provider === 'opencode') {
        const baseUrl = (cfg.opencodeBaseUrl || 'http://localhost:4096').replace(/\/$/, '');
        const model = cfg.opencodeModel || '';
        if (!model)
          throw new Error(
            'OpenCode model not configured. Set opencodeModel in plugin settings (e.g. anthropic/claude-sonnet-4 or your local provider/model id).',
          );
        const username = cfg.opencodeUsername || 'opencode';
        const password = cfg.opencodePassword || '';
        return callOpenCode(baseUrl, model, username, password, messages, system);
      }

      // Default: anthropic
      const apiKey = cfg.anthropicApiKey || '';
      if (!apiKey) throw new Error('Anthropic API key not configured. Add anthropicApiKey in plugin settings.');
      return callAnthropic(apiKey, messages, system);
    }

    // ─────────────────────────────────────────────────────────────────────────

    // AI chat action — proxies messages to the configured AI provider.
    // Keeps API keys server-side; the UI never touches them directly.
    // Returns { text, error?, provider } — never throws.
      ...
      try {
        ...
        if (provider === 'gemini' && !cfg.geminiApiKey) {
          return {
            ok: false,
            provider,
            error: 'Gemini API key not configured. Add geminiApiKey in plugin settings.',
          };
        }
        if (provider === 'openai' && !cfg.openaiApiKey) {
          return {
            ok: false,
            provider,
            error: 'OpenAI API key not configured. Add openaiApiKey in plugin settings.',
          };
        }
        if (provider === 'opencode' && !cfg.opencodeModel) {
          return {
            ok: false,
            provider,
            error:
              'OpenCode model not configured. Set opencodeModel (and opencodeBaseUrl if not localhost:4096) in plugin settings.',
          };
        }
        if (provider === 'anthropic' && !cfg.anthropicApiKey) {
          return {
            ok: false,
            provider,
            error: 'Anthropic API key not configured. Add anthropicApiKey in plugin settings.',
          };
        }
